...
import org.elite.resolver.BeanPropertyELResolver;
import org.elite.resolver.ClassResolver;
import org.elite.resolver.ListELResolver;
import org.elite.resolver.MapELResolver;
import org.elite.resolver.MethodResolver;
import org.elite.resolver.SeqELResolver;
import org.elite.resolver.StringELResolver;
import org.elite.resolver.SystemClassELResolver;
import org.elite.resolver.UnitELResolver;
...
public final class ELEngine {
   ...
   */
  public static Object evaluateExpression(ELContext elctx, String expression,
                                          Class<?> expectedType) {
   ...
   * @return 最佳匹配的方法
   */
  public static Method resolveMethod(ELContext elctx, Class<?> baseClass,
                                     String name, Closure[] args) {
   ...
   * @return 最佳匹配的方法
   */
  public static Method resolveMethod(ELContext elctx, Method[] methods,
                                     String name, Closure[] args) {
    ...
    for (Method method : methods) {
      ...
      if (nargs == argc) {
        if (!method.isVarArgs()) {
          ...
          // match for var-args; the var-args may be passed as an array or
          // individual args
          if (dist != -1) {
    ...
    if (is_vargs) {
      ...
      if (delayed(argtype)) {
        ...
        for (int i = 0; i < vargc; i++) {
          Array.set(vargs, i, coerce(elctx, args[ivarg++].getValue(elctx),
                                     argtype));
        }
        values[nargs] = vargs;
      }
   ...
   * 调用给定目标过程.
   */
  public static Object invokeTarget(ELContext elctx, Object target,
                                    Closure[] args) {
    if (target == null) {
      ...
      Object result = ((ClosureObject)target).invokeSpecial(elctx, "__call__",
                                                            args);
  ...
  public static Object callTarget(ELContext elctx, Object target,
                                  Object... args) {
  ...
  private static Object invokeClass(ELContext elctx, Class<?> target,
                                    Closure[] args) {
  ...
  public static Object resolveDataClass(EvaluationContext context,
                                        String name) {
    if (name == null) {
      return null;
    }
   ...
   * 查找和指定参数相匹配的构造函数.
   */
  public static Constructor<?> resolveConstructor(ELContext elctx, Class<?> cls,
                                                  Closure[] args) {
   ...
   * 值转换成构造方法的实际参数类型.
   */
  public static Object newInstance(ELContext elctx, Class<?> cls,
                                   Closure[] args) {
    try {
  ...
  private static int distanceof(ELContext elctx, Class<?>[] types,
                                Closure[] args, int nargs) {
    int dist = 0;
  ...
  public static final boolean MEASURES_AVAILABLE =
    classPresent("javax.measure.Measure");
}
