...
class ConstantFolder extends TreeTransformer {
  ...
  private static Object nodeValue(ELNode node) {
    ...
    throw new IllegalArgumentException(
      "not a constant: " + node.getClass().getSimpleName());
  }
  ...
  private static ELNode makeConst(int pos, Object value) {
    ...
    throw new IllegalArgumentException(
      "not a constant: " + value.getClass().getSimpleName());
  }

  private static ELNode newInstance(ELNode.Binary e, ELNode left,
                                    ELNode right) {
    if (e instanceof ELNode.ADD)
      return new ELNode.ADD(e.pos, left, right);
    if (e instanceof ELNode.SUB)
      return new ELNode.SUB(e.pos, left, right);
    if (e instanceof ELNode.MUL)
      return new ELNode.MUL(e.pos, left, right);
    if (e instanceof ELNode.IDIV)
      return new ELNode.IDIV(e.pos, left, right);
    if (e instanceof ELNode.DIV)
      return new ELNode.DIV(e.pos, left, right);
    if (e instanceof ELNode.REM)
      return new ELNode.REM(e.pos, left, right);
    if (e instanceof ELNode.POW)
      return new ELNode.POW(e.pos, left, right);
    if (e instanceof ELNode.BITAND)
      return new ELNode.BITAND(e.pos, left, right);
    if (e instanceof ELNode.BITOR)
      return new ELNode.BITOR(e.pos, left, right);
    if (e instanceof ELNode.XOR)
      return new ELNode.XOR(e.pos, left, right);
    if (e instanceof ELNode.SHL)
      return new ELNode.SHL(e.pos, left, right);
    if (e instanceof ELNode.SHR)
      return new ELNode.SHR(e.pos, left, right);
    if (e instanceof ELNode.USHR)
      return new ELNode.USHR(e.pos, left, right);
    if (e instanceof ELNode.EQ)
      return new ELNode.EQ(e.pos, left, right);
    if (e instanceof ELNode.NE)
      return new ELNode.NE(e.pos, left, right);
    if (e instanceof ELNode.IDEQ)
      return new ELNode.IDEQ(e.pos, left, right);
    if (e instanceof ELNode.IDNE)
      return new ELNode.IDNE(e.pos, left, right);
    if (e instanceof ELNode.LT)
      return new ELNode.LT(e.pos, left, right);
    if (e instanceof ELNode.LE)
      return new ELNode.LE(e.pos, left, right);
    if (e instanceof ELNode.GT)
      return new ELNode.GT(e.pos, left, right);
    if (e instanceof ELNode.GE)
      return new ELNode.GE(e.pos, left, right);
    if (e instanceof ELNode.CMP)
      return new ELNode.CMP(e.pos, left, right);
    if (e instanceof ELNode.AND)
      return new ELNode.AND(e.pos, left, right);
    if (e instanceof ELNode.OR)
      return new ELNode.OR(e.pos, left, right);
    if (e instanceof ELNode.CAT)
      return new ELNode.CAT(e.pos, left, right);
    throw new IllegalArgumentException(
      "unsupported binary: " + e.getClass().getSimpleName());
  }

  // ---- Binary arithmetic ----

  private ELNode foldBinaryArithmetic(ELNode.Binary e, ELNode left,
                                      ELNode right) {
  ...
  public void visit(ELNode.ADD e) {
    result = foldBinaryArithmetic(e, transform(e.left), transform(e.right));
  }
  ...
  private ELNode foldBinaryComparison(ELNode.Binary e, ELNode left,
                                      ELNode right) {
    if (isConstant(left) && isConstant(right)) {
      Object lv = nodeValue(left), rv = nodeValue(right);
      try {
  ...
  public void visit(ELNode.EQ e) {
    result = foldBinaryComparison(e, transform(e.left), transform(e.right));
  }
