...
class Grammar implements Serializable {
  ...
  static final int MAXPROD     = 512; // Maximum number of productions in the
                                      // input grammar
   ...
   * into the start state. Consequently, a YYR(0) in the parse table represents
   * an accepting action and the table-generation code doesn't have to treat the
   * accepting action any differently than a normal reduce.
   */

  static final int YY_ACCEPT = 0;                 // accept action
  static final int YY_FAIL   = Integer.MAX_VALUE; // fail action
   ...
   * value used for a token (as used for a column index in the transition
   * matrix) and evaluates to a string naming that token.
   */
  YYSTOK[][] yy_stok;
   ...
   * compressed and can be accessed using the yy_next() subroutine, declared
   * below.
   *
   *             yyg000[]={       5,3    ,    2,2    ,    1,1    };
   * uncovered state-+            | |
  ...
  private Closure make_list(ELContext elctx, YYACT act, int rhs_len) {
    ...
    if (act.optflg == OPT_LIST_HEAD) {
      ...
      lst = (List<Object>)lhs.getValue(elctx);
    }
