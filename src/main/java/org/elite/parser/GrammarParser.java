...
import static org.elite.parser.Grammar.*;
...
public class GrammarParser {
  ...
  static class SYMBOL {
    ...
    Set<YYSTOK> literals;           // terminal token literals
    int         val;                // numeric value of symbol
  ...
  /* The following macros are used to adjust the nonterm values so that
   ...
   */
  static int adj_val(int x) {
    return x - MINNONTERM;
  }

  static int unadj_val(int x) {
    return x + MINNONTERM;
  }
   ...
   */
  private final SYMBOL[] _terms = new SYMBOL[MINACT];

  /* The table that maintains literal terminal symbols. */
  private final Map<YYSTOK, String> _literals = new HashMap<>(157);
  /* The table that maintains terminal symbols. */
  private final Map<String, SYMBOL> _termtab = new HashMap<>(157);
  /* The table that maintains nonterminal symbols. */
  private final Map<String, SYMBOL> _nontermtab = new HashMap<>(157);
  ...
  private final List<ELProgram> _code_blocks = new ArrayList<>();

  /* Lexical fragments */
  private final Map<String, String> _fragments = new HashMap<>(17);
  ...
  private final Parser S;      // The input scanner
  ...
  private void transformer(String id) {
    ...
    /*------------------------------------------------------------------------*/
    /* Action routines. These build up the symbol table from the input
     * specification. */
   ...
   * nested optional (or repeating) productions defined with the [] and []*
   ...
   * pushed and a new right-hand side is started for the subexpression. Note
   * that the first character of the created rhs name (001 in the previous
   * examples) is a space, which is illegal in a user-supplied production name
   * so we don't have to worry about conflicts. Subsequent symbols are added to
   * this new right-hand side. When the ), )?, )*, or )+ is found, we finish the
   * new right-hand side, pop the stack and add the name of the new right-hand
   * side to the previously collected left-hand side.
   */
  ...
  private void end_opt(String id, String sep, int lex) {
    ...
    if (lex == STAR_CLOSE || lex == PLUS_CLOSE) {
      ...
      add_action(null, action, trans,
                 (sep == null) ? OPT_LIST_CONS : OPT_SEP_CONS);
    }
  ...
  private void patch() {
    Map<Integer, YYACT> tab = new HashMap<>();
  ...
  private void make_one_case(int prod_num, int rhs_size, SYMBOL sym,
                             PRODUCTION prod, Map<Integer, YYACT> tab) {
  ...
  // Maximum number of kernel items in a state
  static final int MAXKERNEL = 32;
  // Maximum number of closure items in a state (less the epsilon productions).
  static final int MAXCLOSE = 128;
  // Maximum number of epsilon productions that can be in a closure set for any
  // given state
  static final int MAXEPSILON = 8;
  ...
  /* Array of pointers to the head of the action chains. Indexed by state
   * number. */
  ...
  private final Map<STATE_KEY, STATE> _states = new HashMap<>();
  ...
  static class STATE_KEY {
    ...
    public boolean equals(Object obj) {
      ...
      if (obj instanceof STATE_KEY that) {
  ...
  static final Comparator<ITEM> item_cmp = (item1, item2) -> {
    int rval;
  ...
  private void lr(STATE cur_state) {
    ...
    while ((cur_state = get_unfinished()) != null) {
      ...
      System.arraycopy(closure_items, nitems, closure_items, 0,
                       nclose -= nitems);
   ...
   * (src_items) to the items already in the state (dst_items). false is
   * returned if nothing was done (all lookaheads  in the new state are already
   * in the existing state), true otherwise. It's an internal error if the items
   * don't match.
   */
  private boolean merge_lookaheads(ITEM[] dst_items, ITEM[] src_items,
                                   int nitems) {
   ...
   */
  private int kclosure(STATE kernel, ITEM[] closure_items, int maxitems,
                       int nclose) {
    maxitems -= nclose;
  ...
  private int closure(STATE kernel, ITEM[] closure_items, int maxitems) {
    ...
    for (i = 0; i < kernel.nkitems; i++) {
      if ((n = do_close(kernel.kernel_items[i], closure_items, nclose,
                        maxitems)) != 0) {
        did_something = true;
        if (n > 0) {
          nclose += n;
    ...
    while (did_something) {
      did_something = false;
      for (i = 0; i < nclose; i++) {
        if ((n = do_close(closure_items[i], closure_items, nclose, maxitems))
            != 0) {
          did_something = true;
          if (n > 0) {
            nclose += n;
   ...
   */
  private int do_close(ITEM item, ITEM[] closure_items, int nitems,
                       int maxitems) {
    boolean did_something = false;
   ...
   */
  private ITEM in_closure_items(PRODUCTION production, ITEM[] closure_items,
                                int nitems) {
    for (int i = 0; i < nitems; i++)
    ...
    for (token = item.lookaheads.nextSetBit(0); token >= 0;
      ...
      if ((ap = p_action(state.num, token)) == null) {     // No conflicts
        ...
        System.err.printf("warning: State %2d: reduce/reduce conflict " +
                          "%d/%d on %s (choose %d).\n", state.num, -ap.do_this,
                          item.prod_num,
                          token != 0 ? _terms[token].name : "<EOI>", reduce_by);
      } else {
        ...
        if (pprec != 0 && tprec != 0) {
          ...
          System.err.printf("warning: State %2d: shift/reduce conflict %s/%d" +
                            " (choose %s)\n", state.num, _terms[token].name,
                            item.prod_num, ap.do_this < 0 ? "reduce" : "shift");
        }
      }
    }
  }
  ...
  private static boolean subset(BitSet set, BitSet possible_subset) {
    for (int i = possible_subset.nextSetBit(0); i >= 0;
         i = possible_subset.nextSetBit(i + 1))
      if (!set.get(i))
        return false;
    return true;
  }
  ...
  private static <T> T[] ensure_capacity(T[] data, int min_capacity) {
    ...
    if (min_capacity > old_capacity) {
      ...
      data = (T[])Array.newInstance(data.getClass().getComponentType(),
                                    new_capacity);
