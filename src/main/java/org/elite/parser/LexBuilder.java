...
public class LexBuilder {
  ...
  private final NFA[] _sstack = new NFA[SSIZE];  /* Stack used by new_nfa() */
  ...
  private final Map<String, String> _macros = new HashMap<>();
  private final String[] _mac_stack = new String[SSIZE]; /* Input-source stack */
  private int            _mac_sp = -1;                   /* and stack pointer */

  private final Lexer _lexer = new DefaultLexer();       /* The default lexer */
  ...
  private void parse_error(ERR_NUM type) {
    throw new ParseException(null, Position.line(_pos), Position.column(_pos),
                             type.errmsg);
  }
   ...
   * the actual value. For example, if a "\s" is encountered, Lexeme will hold
   * a space character.  Advance both modifies Current_tok to the current token
   * and returns it.
   *
   * Macro expansion is handled by means of a stack (declared at the top of the
   * subroutine). When an expansion request is encountered, the current input
   * buffer is stacked, and input is read from the macro text. This process
   * repeats with nested macros, so SSIZE controls the maximum macro-nesting
   * depth.
   */
  private int advance() {
    int t, c;
  ...
  private boolean first_in_cat(int tok) {
    ...
    case PLUS_CLOSE:
    case OPTIONAL:
      parse_error(ERR_NUM.E_CLOSE);
      return false;

    case CCL_END:
      parse_error(ERR_NUM.E_BRACKET);
      return false;
    }
  ...
  private NFA term() {
    ...
    if (_current_tok == OPEN_PAREN) {
      ...
      if (!(_current_tok == ANY || _current_tok == CCL_START ||
            _current_tok == PCCL)) {
        start.edge = _lexeme;
        advance();
  ...
  private static class MachineLexer extends DefaultLexer {
    ...
    private final Map<BitSet, DFA> dstates = new HashMap<>();
    private final Stack<NFA>       stack = new Stack<>();
