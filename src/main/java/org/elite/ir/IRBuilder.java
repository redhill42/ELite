...
public class IRBuilder extends ELNode.Visitor {
  ...
  public void visit(ELNode.APPLY node) {
    ...
    if (base instanceof ELNode.ACCESS acc) {
      ...
      build(node.args);
      current.emitNewTuple(node.args.length);
      emitInvokeStatic(Runtime.class, "invoke", ELContext.class,
  ...
  private void inlineInstructions(SymbolTable.Symbol sym, Slot[] slots,
                                  ELNode[] args) {
    IRFunction fn = sym.func;
  ...
  public void visit(ELNode.ARRAY node) {
    ...
    emitInvokeStatic(Runtime.class, "newArray", ELContext.class,
                     Object.class, Object[].class, Object[].class);
  }
    ...
    if (node.dims == null || node.dims.length == 1) {
      ...
      if (node.init != null) {
        ...
        for (int i = 0; i < node.init.length; i++) {
          ...
          emitInvokeStatic(Array.class, "set", Object.class,
  ...
  public void visit(ELNode.INSTANCEOF node) {
    if (node.type.indexOf('.') == -1) {
      ...
      if (sym != null && sym.def.expr instanceof ELNode.CLASSDEF) {
        emitPushSymbol(sym);
        build(node.right);
        emitInvokeMethod(ClassDefinition.class, "isInstance",
                         ELContext.class, Object.class);
        if (node.negative)
          current.emitNot();
        return;
  ...
  public void visit(ELNode.ASSIGNOP node) {
    ...
    build(node.left);
    build(node.right);
    emitInvokeStatic(Runtime.class, "invokeAssignOp", ELContext.class,
  ...
  public void visit(ELNode.NEW node) {
    ...
    // elite class or a data class. First let me try Java class, then lookup
    // symbol to see if the target is a CLASSDEF, then fallback to trampoline.
