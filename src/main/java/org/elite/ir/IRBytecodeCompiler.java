...
public class IRBytecodeCompiler {
  ...
  private static final int A_ALOAD = 25, A_ARETURN = 176, A_RETURN = 177,
    A_ASTORE = 58, A_ACONST_NULL = 1;
  private static final int A_ICONST_0 = 3, A_ICONST_1 = 4, A_LCONST_0 = 9,
    A_LCONST_1 = 10;
  private static final int A_DCONST_0 = 14, A_DCONST_1 = 15;
  private static final int A_BIPUSH = 16, A_SIPUSH = 17;
  private static final int A_IADD = 96, A_ISUB = 100, A_IMUL = 104, A_IDIV
    = 108, A_IREM = 112, A_INEG = 116;
  private static final int A_LADD = 97, A_LSUB = 101, A_LMUL = 105, A_LDIV
    = 109, A_LREM = 113, A_LNEG = 117;
  private static final int A_DADD = 99, A_DSUB = 103, A_DMUL = 107, A_DDIV
    = 111, A_DNEG = 119;
  private static final int A_DCMPG = 152, A_LCMP = 148;
  private static final int A_IF_ICMPEQ = 159, A_IF_ICMPNE = 160, A_IF_ICMPLT
    = 161;
  private static final int A_IF_ICMPLE = 164, A_IF_ICMPGT = 163, A_IF_ICMPGE
    = 162;
  private static final int A_IF_ACMPEQ = 165, A_IF_ACMPNE = 166;
  private static final int A_IFEQ = 153, A_IFNE = 154, A_IFLT = 155, A_IFLE
    = 158, A_IFGT = 157, A_IFGE = 156;
  private static final int A_INVOKESPECIAL = 183, A_INVOKESTATIC = 184,
    A_INVOKEVIRTUAL = 182;
  ...
  private static final String EXECUTE_DESC = "(" + ELCTX_DESC + LOCALS_DESC +
                                             ")Ljava/lang/Object;";
  ...
  private static class SingleLoader extends ClassLoader {
    SingleLoader() {
      super(IRBytecodeCompiler.class.getClassLoader());
    }
  ...
  public static IRCompiledFunction compile(IRFunction fn) {
    ...
    // Register IRFunction constant pool so CLOSURE bytecode can look up via
      // funcIdx
    ...
    try {
      ...
      java.lang.reflect.Method m = c.getMethod("execute",
                                               ...
                                               Object[].class);
      return new IRCompiledFunction(m, bc, name, fn.maxLocals(),
                                    fn.defaultValues());
    } catch (Exception e) {
  ...
  private IRBytecodeCompiler(IRFunction fn, String className) {
    ...
    cm.visitMethodInsn(A_INVOKESPECIAL, "java/lang/Object", "<init>", "()V",
                       false);
  ...
  private void compileBlock(int blockId) {
    ...
    int end = (blockId + 1 < fn.blockCount()) ? fn.blockStart(blockId + 1)
                                              : fn.code().length;
  ...
  private void compileInst(InstructionView v) {
    ...
    case PUSH_TRUE -> {
      ...
      mv.visitMethodInsn(A_INVOKESTATIC, "java/lang/Boolean", "valueOf",
    ...
    case THROW -> {
      ...
      mv.visitMethodInsn(A_INVOKESTATIC, "org/elite/eval/Runtime", "wrapThrow",
                         "(Ljava/lang/Object;)Ljava/lang/RuntimeException;",
                         false);
    ...
    case JUMP_IF_NULL ->
      mv.visitJumpInsn(198, blockLabels[v.jumpTarget()]); // IFNULL
    case JUMP_IF_NONNULL ->
      mv.visitJumpInsn(199, blockLabels[v.jumpTarget()]); // IFNONNULL
    ...
    case LOAD_FIELD -> {
      ...
      mv.visitMethodInsn(A_INVOKESTATIC, "org/elite/eval/Runtime", "loadField",
    ...
    case STORE_FIELD -> {
      String name = (String)fn.constantPool()[v.poolIndex()];
      // Stack: [value, base]. LDC name → [value, base, name] — name on top
        // (3rd param ✓)
      mv.visitLdcInsn(name);
      mv.visitMethodInsn(A_INVOKESTATIC, "org/elite/eval/Runtime",
                         "storeFieldBC",
    ...
    case PUSH_GLOBAL -> {
      ...
      mv.visitMethodInsn(A_INVOKESTATIC, "org/elite/eval/Runtime", "pushGlobal",
                         "(Ljavax/el/ELContext;Ljava/lang/String;)" +
                         "Ljava/lang/Object;",
                         false);
    }
    ...
    // ─── Collections ───
    case NEW_MAP ->
      emitCallN("newMap", pl * 2); // pl = pair count, 2 stack values per pair
    ...
    case NIL ->
      mv.visitMethodInsn(A_INVOKESTATIC, "org/elite/eval/seq/Cons", "nil",
                         "()Lorg/elite/eval/seq/Cons;");

    case INVOKE_GETTER -> {
      java.lang.reflect.Method m
        = (java.lang.reflect.Method)fn.constantPool()[v.poolIndex()];
      emitDirectGetter(m);
    }
    case CLOSURE -> {
      int funcIdx = v.poolIndex();
      ...
      mv.visitMethodInsn(A_INVOKESTATIC, "org/elite/eval/Runtime",
                         "createClosureById",
                         "(Ljavax/el/ELContext;I[Ljava/lang/Object;)" +
                         "Lorg/elite/ir/IRClosure;",
                         false);
    }
    ...
    case TRAMPOLINE -> {
      ...
      mv.visitMethodInsn(A_INVOKESTATIC, "org/elite/eval/Runtime",
                         "trampolineById",
                         "(Ljavax/el/ELContext;I)Ljava/lang/Object;", false);
    }
    case NOP -> {
    }
    // Dynamic ops: call static helper methods directly
    case ADD, SUB, MUL, DIV, REM, POW, CAT, IN, SHL, SHR, USHR, EQ, NE, LT, LE,
         GT, GE, BITAND, BITOR, XOR -> {
    ...
    default -> throw new CompilationError(
      _T(IR_BC_UNHANDLED_OPCODE, Opcode.name(op), op));
    }
  }
  ...
  /**
   * Reference identity comparison (=== / !==). Two Object refs on stack →
   * Boolean.
   */
  ...
  private void emitUnboxInt(int count) {
    ...
    if (count == 2) {
      ...
      mv.visitMethodInsn(A_INVOKEVIRTUAL, "java/lang/Number", "intValue", "()I",
                         false);
  ...
  private final Map<IRFunction, IRCompiledFunction> calleeCache
    = new HashMap<>();
  ...
  /**
   * Pack args, call invokeDirect(funcId, Object[]) at runtime (supports
   * typed optimization).
   */
  private void emitPackArgsAndCall(int argc, boolean direct, int funcId) {
    ...
    if (argc == 0) {
      ...
      for (int i = argc - 1; i >= 0; i--)
        mv.visitVarInsn(A_ASTORE, tempSlots[i]);
      emitIntConst(argc);
      mv.visitTypeInsn(A_ANEWARRAY, "java/lang/Object");
      for (int i = 0; i < argc; i++) {
        mv.visitInsn(A_DUP);
   ...
   */
  private void emitPackArgsAndCall(int argc, boolean direct,
                                   IRCompiledFunction cf) {
    ...
    if (direct && cf != null) {
      ...
      mv.visitMethodInsn(A_INVOKESTATIC, "org/elite/eval/Runtime", "invokeDyn",
                         "(Ljavax/el/ELContext;Ljava/lang/Object;" +
                         "[Ljava/lang/Object;)Ljava/lang/Object;",
                         false);
    } else {
  ...
  private static final ThreadLocal<java.util.Map<Integer, IRFunction>>
    funcRegistry = ThreadLocal.withInitial(java.util.HashMap::new);
  ...
  private static java.util.Map<Integer, IRFunction> funcRegistry() {
    return funcRegistry.get();
  }

  // registerFunction removed — use compileOrGet + calleeCache instead

  // Cache of compiled functions for fast direct calls
  private static final ThreadLocal<java.util.Map<IRFunction,
    IRCompiledFunction>>
    compiledCache = ThreadLocal.withInitial(java.util.HashMap::new);

  /**
   * Direct call: use compiled version if available, specialize on first call.
   */
  public static Object invokeDirect(javax.el.ELContext elctx, int funcId,
                                    Object[] args) {
    IRFunction fn = funcRegistry().get(funcId);
    if (fn == null)
      throw new RuntimeException(_T(IR_FUNCTION_NOT_REGISTERED, funcId));
    // Check cache first
  ...
  private void emitCall2(String method) {
    mv.visitMethodInsn(A_INVOKESTATIC, "org/elite/eval/Runtime", method,
                       "(Ljava/lang/Object;Ljava/lang/Object;)" +
                       "Ljava/lang/Object;",
                       false);
  }
  ...
  private void emitCallN(String method, int count) {
    ...
    if (count == 0) {
      ...
      for (int i = count - 1; i >= 0; i--)
        mv.visitVarInsn(A_ASTORE, slots[i]);
      emitIntConst(count);
      mv.visitTypeInsn(A_ANEWARRAY, "java/lang/Object");
      for (int i = 0; i < count; i++) {
        mv.visitInsn(A_DUP);
        emitIntConst(i);
        mv.visitVarInsn(A_ALOAD, slots[i]);
        mv.visitInsn(A_AASTORE);
      }
    }
    mv.visitMethodInsn(A_INVOKESTATIC, "org/elite/eval/Runtime", method,
  ...
  private void emitDirectMethod(java.lang.reflect.Method m, int argc) {
    ...
    int invokeOp = isStatic ? A_INVOKESTATIC
                            : isInterface ? A_INVOKEINTERFACE : A_INVOKEVIRTUAL;
    mv.visitMethodInsn(invokeOp, owner, name, desc, isInterface);
    emitBoxIfPrimitive(returnType);
  }
  ...
  private void emitBoxIfPrimitive(Class<?> type) {
    if (type == int.class) {
      mv.visitMethodInsn(A_INVOKESTATIC, "java/lang/Integer", "valueOf",
                         "(I)Ljava/lang/Integer;", false);
    } else if (type == long.class) {
      mv.visitMethodInsn(A_INVOKESTATIC, "java/lang/Long", "valueOf",
                         "(J)Ljava/lang/Long;", false);
    } else if (type == double.class) {
      mv.visitMethodInsn(A_INVOKESTATIC, "java/lang/Double", "valueOf",
                         "(D)Ljava/lang/Double;", false);
    } else if (type == float.class) {
      mv.visitMethodInsn(A_INVOKESTATIC, "java/lang/Float", "valueOf",
                         "(F)Ljava/lang/Float;", false);
    } else if (type == boolean.class) {
      mv.visitMethodInsn(A_INVOKESTATIC, "java/lang/Boolean", "valueOf",
                         "(Z)Ljava/lang/Boolean;", false);
    } else if (type == short.class) {
      mv.visitMethodInsn(A_INVOKESTATIC, "java/lang/Short", "valueOf",
                         "(S)Ljava/lang/Short;", false);
    } else if (type == byte.class) {
      mv.visitMethodInsn(A_INVOKESTATIC, "java/lang/Byte", "valueOf",
                         "(B)Ljava/lang/Byte;", false);
    } else if (type == char.class) {
      mv.visitMethodInsn(A_INVOKESTATIC, "java/lang/Character", "valueOf",
  ...
  private void emitUnboxIfPrimitive(Class<?> type) {
    if (type == int.class) {
      ...
      mv.visitTypeInsn(A_CHECKCAST, "java/lang/Number");
      mv.visitMethodInsn(A_INVOKEVIRTUAL, "java/lang/Number", "longValue",
                         "()J", false);
    } else if (type == double.class) {
      mv.visitTypeInsn(A_CHECKCAST, "java/lang/Number");
      mv.visitMethodInsn(A_INVOKEVIRTUAL, "java/lang/Number", "doubleValue",
                         "()D", false);
    } else if (type == float.class) {
      mv.visitTypeInsn(A_CHECKCAST, "java/lang/Number");
      mv.visitMethodInsn(A_INVOKEVIRTUAL, "java/lang/Number", "floatValue",
                         "()F", false);
    } else if (type == boolean.class) {
      mv.visitTypeInsn(A_CHECKCAST, "java/lang/Boolean");
      mv.visitMethodInsn(A_INVOKEVIRTUAL, "java/lang/Boolean", "booleanValue",
                         "()Z", false);
    } else if (type == short.class) {
      mv.visitTypeInsn(A_CHECKCAST, "java/lang/Number");
      mv.visitMethodInsn(A_INVOKEVIRTUAL, "java/lang/Number", "shortValue",
                         "()S", false);
    } else if (type == byte.class) {
      mv.visitTypeInsn(A_CHECKCAST, "java/lang/Number");
      mv.visitMethodInsn(A_INVOKEVIRTUAL, "java/lang/Number", "byteValue",
                         "()B", false);
    } else if (type == char.class) {
      mv.visitTypeInsn(A_CHECKCAST, "java/lang/Character");
      mv.visitMethodInsn(A_INVOKEVIRTUAL, "java/lang/Character", "charValue",
                         "()C", false);
  ...
  /**
   * Compute JVM local var slot from IR var index (accounting for category-2
   * types).
   */
  ...
  private void emitDynBinOp(String method) {
    String desc
      = "(Ljavax/el/ELContext;Ljava/lang/Object;Ljava/lang/Object;)" +
        "Ljava/lang/Object;";
    mv.visitMethodInsn(A_INVOKESTATIC, "org/elite/eval/Runtime", method, desc,
                       false);
  }
  ...
  private void emitIntConst(int i) {
    if (i >= -1 && i <= 5)
      mv.visitInsn(A_ICONST_0 + i);
    else if (i >= Byte.MIN_VALUE && i <= Byte.MAX_VALUE)
      mv.visitIntInsn(A_BIPUSH, i);
    else if (i >= Short.MIN_VALUE && i <= Short.MAX_VALUE)
      mv.visitIntInsn(A_SIPUSH, i);
    else
      mv.visitLdcInsn(i);
  }
  ...
  private void emitLongConst(long l) {
    if (l == 0)
      mv.visitInsn(A_LCONST_0);
    else if (l == 1)
      mv.visitInsn(A_LCONST_1);
    else
      mv.visitLdcInsn(l);
  }
  ...
  private void emitDoubleConst(double d) {
    if (d == 0.0)
      mv.visitInsn(A_DCONST_0);
    else if (d == 1.0)
      mv.visitInsn(A_DCONST_1);
    else
      mv.visitLdcInsn(d);
  }
}
