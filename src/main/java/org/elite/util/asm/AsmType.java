...
public class AsmType {
   ...
   * The <tt>void</tt> type.
   */
  public final static AsmType VOID_TYPE = new AsmType(VOID);

  /**
   * The <tt>boolean</tt> type.
   */
  public final static AsmType BOOLEAN_TYPE = new AsmType(BOOLEAN);

  /**
   * The <tt>char</tt> type.
   */
  public final static AsmType CHAR_TYPE = new AsmType(CHAR);

  /**
   * The <tt>byte</tt> type.
   */
  public final static AsmType BYTE_TYPE = new AsmType(BYTE);

  /**
   * The <tt>short</tt> type.
   */
  public final static AsmType SHORT_TYPE = new AsmType(SHORT);

  /**
   * The <tt>int</tt> type.
   */
  public final static AsmType INT_TYPE = new AsmType(INT);

  /**
   * The <tt>float</tt> type.
   */
  public final static AsmType FLOAT_TYPE = new AsmType(FLOAT);

  /**
   * The <tt>long</tt> type.
   */
  public final static AsmType LONG_TYPE = new AsmType(LONG);

  /**
   * The <tt>double</tt> type.
   */
  public final static AsmType DOUBLE_TYPE = new AsmType(DOUBLE);

  /**
   * The <tt>byte[]</tt> type.
   */
  public final static AsmType BYTE_ARRAY_TYPE = getType(byte[].class);
   ...
   */
  private AsmType(final int sort, final char[] buf, final int off,
                  final int len) {
    this.sort = sort;
    this.buf = buf;
    this.off = off;
   ...
   * @return the {@link Type#OBJECT} type for the given class name.
   */
  public static AsmType getObjectType(String name) {
    int l = name.length();
    char[] buf = new char[l + 2];
    buf[0] = 'L';
    buf[l + 1] = ';';
    name.getChars(0, l, buf, 1);
    return new AsmType(OBJECT, buf, 0, l + 2);
  }
  ...
  private static AsmType getType(final char[] buf, final int off) {
    ...
    default:
      ...
      return new AsmType(OBJECT, buf, off, len + 1);
    }
  }
  ...
  public static String toInternalName(Class<?> c) {
    return (c == null) ? null : c.getName().replace('.', '/');
  }

  public static String[] toInternalName(String[] names) {
    if (names == null) {
      return null;
    }
    ...
    for (int i = 0; i < names.length; i++) {
      result[i] = names[i].replace('.', '/');
    }
    return result;
  }

  public static String[] toInternalName(Class<?>[] classes) {
    if (classes == null) {
      return null;
    }
    ...
    for (int i = 0; i < classes.length; i++) {
      result[i] = classes[i].getName().replace('.', '/');
    }
    return result;
  }
  ...
  public static String toMethodDescriptor(String ret, String... names) {
    StringBuilder result = new StringBuilder();
    result.append("(");
    for (String name : names)
      result.append(toDescriptor(name));
    result.append(")");
    result.append(ret);
  ...
  public static String getMethodDescritpor(final Class<?> returnType,
    ...
    if (argumentTypes != null) {
      for (Class<?> argumentType : argumentTypes) {
        getDescriptor(buf, argumentType);
      }
    }
    buf.append(')');
  ...
  public static int getSize(Class<?> c) {
    return c == Long.TYPE || c == Double.TYPE ? 2 : 1;
  }
   ...
   *               ISTORE, IALOAD, IASTORE, IADD, ISUB, IMUL, IDIV, IREM,
   *               INEG, ISHL, ISHR, IUSHR, IAND, IOR, IXOR and IRETURN.
  ...
  public boolean equals(final Object o) {
    ...
    if (sort == AsmType.OBJECT || sort == AsmType.ARRAY) {
      if (len != t.len) {
        return false;
      }
